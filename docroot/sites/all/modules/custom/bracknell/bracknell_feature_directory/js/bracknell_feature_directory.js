  ...
  Drupal.behaviors.bracknell_feature_directory_parks_show_more_button = {
    attach: function (context, settings) {
      if ($('[data-js="showcase-reveal"]', context).length > 0) {
        ...
        var totalItems = self.find('.showcase .views-row').length;
        ...
        if (totalItems > 9) {
          ...
          var locationType = self.attr('data-title');
          var displayButtonTextHidden = Drupal.t('More @type', { '@type': locationType });
          var displayButtonTextShown = Drupal.t('Fewer @type', { '@type': locationType });

          var buttonMarkup = '<span class="button-text">' +
            displayButtonTextHidden +
            ' </span> '+
          ...
          self.find('.showcase .views-row')
            .filter(':eq(9), :gt(9)')
            .first()
            .addClass('showcase-first-item-hidden')
            .attr('tabindex', '-1')
            .css('outline', 'none');
          ...
          displayButton.on('click', function () {
            if ($(this).attr('aria-expanded') === 'false') {
              ...
              $(this).find('.button-text')
                .text(displayButtonTextShown);

              $(this).find('.icon')
                .addClass('theme-icon-triangle-up')
                .removeClass('theme-icon-triangle-down');

              // Move keyboard focus to the first revealed item.
              self.find('.showcase-first-item-hidden')
                .focus();
            }
            else {
              self.find('.showcase .views-row')
              ...
              $(this).find('.button-text')
                .text(displayButtonTextHidden);

              $(this).find('.icon')
                .addClass('theme-icon-triangle-down')
                .removeClass('theme-icon-triangle-up');
            }
          });
        }
      }
